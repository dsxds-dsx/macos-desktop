window.renderBooks = function(body, sidebar, toolbar, windowId) {
    // ============ Persistent State ============
    let state = JSON.parse(localStorage.getItem('macos_books_state') || 'null') || {
    ...
    function saveState() {
        localStorage.setItem('macos_books_state', JSON.stringify(state));
    }
    ...
    // ============ Sidebar Sections ============
    const sections = [
        { id: 'library', name: '书库', icon: 'books' },
        { id: 'reading', name: '正在阅读', icon: 'book-open' },
        { id: 'finished', name: '已读完', icon: 'check' },
        { id: 'bookstore', name: '书店', icon: 'store' },
        { id: 'bookmarks', name: '书签', icon: 'bookmark' }
    ];
    ...
    function renderSidebar() {
        if (!sidebar) return;
        // load fresh progress
        books.forEach(b => b.progress = loadProgress(b.id));
        const readingCount = books.filter(b => b.progress > 0 && b.progress < 100).length;
        const finishedCount = books.filter(b => b.progress >= 100).length;

        sidebar.innerHTML = `
            <div class="books-sidebar">
                <div class="books-sidebar-top">
                    <div class="books-sidebar-title">图书</div>
                    <div class="books-sidebar-search">
                        ...
                        <input type="text" id="books-search-${windowId}" placeholder="搜索书库" value="${escapeHtml(searchQuery)}">
                    </div>
                </div>
                <div class="books-sidebar-scroll">
                    ${sections.map(sec => {
                        let badge = '';
                        if (sec.id === 'reading' && readingCount > 0) badge = readingCount;
                        else if (sec.id === 'finished' && finishedCount > 0) badge = finishedCount;
                        return `
                            <div class="books-sidebar-section ${state.currentSection === sec.id ? 'active' : ''}" data-section="${sec.id}">
                                ${sectionIcons[sec.id]}
                                <span>${sec.name}</span>
                                ${badge ? `<span class="books-sidebar-badge">${badge}</span>` : ''}
                            </div>
                        `;
                    }).join('')}
                </div>
        ...
        const searchInput = sidebar.querySelector(`#books-search-${windowId}`);
        if (searchInput) {
    ...
    function renderContent() {
        if (state.readingView && state.selectedBookId) {
            renderReadingView();
            return;
        }
    ...
    function renderBookList() {
        ...
        if (searchQuery.trim()) {
            ...
            displayBooks = books.filter(b => b.title.toLowerCase().includes(q) || b.author.toLowerCase().includes(q));
            title = `搜索"${searchQuery}"`;
        } else if (state.currentSection === 'reading') {
            displayBooks = books.filter(b => b.progress > 0 && b.progress < 100);
            title = '正在阅读';
        } else if (state.currentSection === 'finished') {
            displayBooks = books.filter(b => b.progress >= 100);
            title = '已读完';
        } else if (state.currentSection === 'bookstore') {
            title = '书店';
        }

        if (state.currentSection === 'bookstore' && !searchQuery.trim()) {
            body.innerHTML = `
            ...
            body.innerHTML = `
                <div class="books-content">
                    <div class="books-empty">
                        <div class="books-empty-icon">${emptyIcon}</div>
                        <div class="books-empty-text">${emptyText}</div>
                    </div>
                </div>
            `;
            return;
        ...
        body.innerHTML = `
            <div class="books-content">
                <div class="books-content-inner">
                    <h1 class="books-page-title">${escapeHtml(title)}</h1>
                    <div class="books-grid">
                        ${displayBooks.map(book => `
                            <div class="books-card" data-book="${book.id}">
                                <div class="books-cover" style="background:${book.color};">
                                    <span class="books-cover-icon">${book.cover}</span>
                                    ${book.progress > 0 && book.progress < 100 ? `
                                        <div class="books-cover-progress">
                                            <div class="books-cover-progress-fill" style="width:${book.progress}%;"></div>
                                        </div>
                                    ` : ''}
                                    ${book.progress >= 100 ? `<div class="books-cover-finished">✓</div>` : ''}
                                </div>
                                <div class="books-card-title">${escapeHtml(book.title)}</div>
                                <div class="books-card-author">${escapeHtml(book.author)}</div>
                            </div>
                        `).join('')}
                    </div>
                </div>
        ...
        body.querySelectorAll('[data-book]').forEach(card => {
    ...
    function renderBookDetail() {
        ...
        const content = bookContents[state.selectedBookId];
        if (!book) {
            state.selectedBookId = null;
            renderContent();
            return;
        }
        book.progress = loadProgress(book.id);

        body.innerHTML = `
            <div class="books-content">
                <div class="books-detail">
                    <button class="books-back-btn" id="books-back-${windowId}">
                    ...
                    <div class="books-detail-main">
                        <div class="books-cover-lg" style="background:${book.color};">
                            <span class="books-cover-lg-icon">${book.cover}</span>
                            ${book.progress >= 100 ? `<div class="books-cover-finished-lg">✓ 已读完</div>` : ''}
                        </div>
                        <div class="books-detail-info">
                            <h1 class="books-detail-title">${escapeHtml(book.title)}</h1>
                            <div class="books-detail-author">${escapeHtml(book.author)}</div>
                            ...
                            ` : ''}
                            <button class="books-read-btn" id="books-read-${windowId}">
                                ...
                                <div class="books-detail-toc">
                                    ...
                                    ${content.chapters.map((ch, i) => `
                                        <div class="books-toc-item" data-chapter="${i}">
                                            <span class="books-toc-num">${i + 1}</span>
                                            <span class="books-toc-name">${escapeHtml(ch.title)}</span>
                                            <svg class="books-toc-arrow" viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9 18l6-6-6-6"/></svg>
                                        </div>
                                    `).join('')}
                                </div>
                            ` : ''}
                            <div class="books-detail-desc">
                                ...
                                <div class="books-desc-text">${escapeHtml(bookDescs[book.id] || '一本精彩的书籍，值得细细品读。')}</div>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        body.querySelector(`#books-back-${windowId}`).addEventListener('click', () => {
            state.selectedBookId = null;
            saveState();
            renderContent();
        });

        body.querySelector(`#books-read-${windowId}`).addEventListener('click', () => {
            if (book.progress >= 100) {
                state.currentChapter = 0;
                state.currentPage = 0;
            } else if (content) {
                const targetChapter = Math.floor((book.progress / 100) * content.chapters.length);
                state.currentChapter = Math.min(targetChapter, content.chapters.length - 1);
                state.currentPage = 0;
            }
        ...
        body.querySelectorAll('[data-chapter]').forEach(item => {
            item.addEventListener('click', () => {
                state.currentChapter = parseInt(item.dataset.chapter);
                state.currentPage = 0;
                state.readingView = true;
                saveState();
                renderReadingView();
    ...
    function renderReadingView() {
        ...
        const pageParaStart = state.currentPage * 3;
        const pageParas = paragraphs.slice(pageParaStart, pageParaStart + 3);
        const totalProgress = Math.round(((state.currentChapter + (state.currentPage + 1) / totalPages) / content.chapters.length) * 100);

        body.innerHTML = `
            <div class="books-reader">
                <div class="books-reader-toolbar">
                    <button class="books-reader-back" id="reader-back-${windowId}">
                        <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 12H5M12 19l-7-7 7-7"/></svg>
                        <span>书库</span>
                    </button>
                    <div class="books-reader-info">
                        <span class="books-reader-title">${escapeHtml(book.title)}</span>
                        <span class="books-reader-chapter">${escapeHtml(chapter.title)}</span>
                    </div>
                    <div class="books-reader-controls">
                        <button class="books-reader-icon-btn" id="font-decrease-${windowId}" title="缩小字体">
                            <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M4 7V4h16v3M9 20h6M12 4v16"/></svg>
                        </button>
                        <span class="books-reader-font-size">${state.fontSize}px</span>
                        <button class="books-reader-icon-btn" id="font-increase-${windowId}" title="放大字体">
                            <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M4 7V4h16v3M9 20h6M12 4v16"/></svg>
                        </button>
                        <div class="books-reader-divider"></div>
                        <button class="books-reader-icon-btn" id="bookmark-toggle-${windowId}" title="书签">
                            <svg viewBox="0 0 24 24" width="14" height="14" fill="${state.bookmarks.includes(`${book.id}-${state.currentChapter}`) ? 'currentColor' : 'none'}" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/></svg>
                        </button>
                    </div>
                </div>
                <div class="books-reader-scroll" id="reader-scroll-${windowId}">
                    <div class="books-reader-page" style="font-size:${state.fontSize}px;">
                        <h2 class="books-chapter-title">${escapeHtml(chapter.title)}</h2>
                        ${pageParas.map(p => `<p class="books-paragraph">${escapeHtml(p.trim())}</p>`).join('')}
                    </div>
                </div>
                <div class="books-reader-nav">
                    <button class="books-reader-nav-btn ${state.currentChapter === 0 && state.currentPage === 0 ? 'disabled' : ''}" id="prev-chapter-${windowId}">
                    ...
                    <div class="books-reader-progress">
                        ...
                        <span class="books-progress-text">${totalProgress}% · 第 ${state.currentPage + 1} / ${totalPages} 页</span>
                    </div>
                    <button class="books-reader-nav-btn ${state.currentChapter === content.chapters.length - 1 && state.currentPage >= totalPages - 1 ? 'disabled' : ''}" id="next-chapter-${windowId}">
                        <span>下一章</span>
        ...
        body.querySelector(`#reader-back-${windowId}`).addEventListener('click', () => {
            state.readingView = false;
            saveState();
            renderContent();
        });

        body.querySelector(`#font-decrease-${windowId}`).addEventListener('click', () => {
        ...
        body.querySelector(`#font-increase-${windowId}`).addEventListener('click', () => {
        ...
        body.querySelector(`#bookmark-toggle-${windowId}`).addEventListener('click', () => {
        ...
        body.querySelector(`#prev-chapter-${windowId}`).addEventListener('click', () => {
            if (state.currentPage > 0) {
                state.currentPage--;
            } else if (state.currentChapter > 0) {
                state.currentChapter--;
                const prevChapter = content.chapters[state.currentChapter];
                const prevParas = prevChapter.content.split('\n\n');
                state.currentPage = Math.max(0, Math.ceil(prevParas.length / 3) - 1);
            }
            saveState();
            renderReadingView();
        });

        body.querySelector(`#next-chapter-${windowId}`).addEventListener('click', () => {
            const chapterParas = chapter.content.split('\n\n');
            const chPages = Math.max(1, Math.ceil(chapterParas.length / 3));
            if (state.currentPage < chPages - 1) {
                state.currentPage++;
            } else if (state.currentChapter < content.chapters.length - 1) {
                state.currentChapter++;
                state.currentPage = 0;
            }
            saveState();
        ...
        saveProgress(state.selectedBookId, totalProgress);
    }
